...
if('TSS'%in%readtypes){
	...
	TSS_scaling_factor = sum(metaplot_table[,"TSS_start"]+TSS_optimum) / sum(metaplot_table[,"TSS_start"])
...
if('TES'%in%readtypes){
	...
	TES_scaling_factor = sum(metaplot_table[,"TES_end"]+TES_optimum) / sum(metaplot_table[,"TES_end"])
